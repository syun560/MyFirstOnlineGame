...
function moveInClient(myPlayerObj, missilesMap) {
    ...
    try {
        for (var _iterator = missilesMap[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function drawMap(gameObj) {
    ...
    try {
        for (var _iterator2 = gameObj.playersMap[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            var distanceObj = calcDistance(gameObj.myPlayerObj.x, gameObj.myPlayerObj.y, tekiPlayerObj.x, tekiPlayerObj.y, gameObj.fieldWidth, gameObj.fieldHeight, gameObj.raderCanvasWidth, gameObj.raderCanvasHeight);
    ...
    try {
        for (var _iterator3 = gameObj.itemsMap[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            ...
            var distanceObj = calcDistance(gameObj.myPlayerObj.x, gameObj.myPlayerObj.y, item.x, item.y, gameObj.fieldWidht, gameObj.fieldHeight, gameObj.raderCanvasWidth, gameObj.raderCanvasHeight);
    ...
    try {
        for (var _iterator4 = gameObj.airMap[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            ...
            var distanceObj = calcDistance(gameObj.myPlayerObj.x, gameObj.myPlayerObj.y, air.x, air.y, gameObj.fieldWidht, gameObj.fieldHeight, gameObj.raderCanvasWidth, gameObj.raderCanvasHeight);
    ...
    try {
        for (var _iterator5 = gameObj.missilesMap[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
            var _ref9 = _step5.value;

            var _ref10 = _slicedToArray(_ref9, 2);

            var missileId = _ref10[0];
            var missile = _ref10[1];


            var distanceObj = calcDistance(gameObj.myPlayerObj.x, gameObj.myPlayerObj.y, missile.x, missile.y, gameObj.fieldWidth, gameObj.fieldHeight, gameObj.raderCanvasWidth, gameObj.raderCanvasHeight);

            if (distanceObj.distanceX <= gameObj.raderCanvasWidth / 2 + 50 && distanceObj.distanceY <= gameObj.raderCanvasHeight / 2 + 50) {

                if (missile.emitPlayerId === gameObj.myPlayerObj.playerId) {
                    // 自分自身のミサイルの描画

                    var rotationDeg = gameObj.rotationDegByMissileDirection[missile.direction];
...
function drawPlayer(ctxRader, myPlayerObj) {
    ...
    var rotationDeg = gameObj.rotationDegByDirection[myPlayerObj.direction];
    ...
    ctxRader.drawImage(gameObj.playerImage, -(gameObj.playerImage.width / 2), -(gameObj.playerImage.height / 2));
    ctxRader.restore();
}
...
function drawBom(ctxRader, drawX, drawY, deadCount) {
    ...
    var drawBomNumber = Math.floor(deadCount / 5);
...
socket.on('map data', function (compressed) {
    ...
    var missilesArray = compressed[3];
    ...
    missilesArray.forEach(function (compressedMissileData, index) {
...
function calcDistance(pX, pY, oX, oY, gameWidth, gameHeight, raderCanvasWidth, raderCanvasHeight) {
    ...
    var deg = calcTwoPointsDeg(drawX, drawY, raderCanvasWidth / 2, raderCanvasHeight / 2);
...
function calcTwoPointsDeg(x1, y1, x2, y2) {
    ...
    var deg = radian * 180 / Math.PI + 180;
    return deg;
}
